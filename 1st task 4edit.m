...
betta=0.06;%multiplication factor of antigens;
gamma=0.00003;%coefficient associated with the probability of neutralization of the antigen by antibodies
etta=0.1;%the number of antibodies required to neutralize a single antigen;
sigma=0.000001;%some constant, its own for each disease
...
n=(tn-t0)/dt;
T_cells=1;%life Time Plasma Cells
miu_c=1/T_cells;
T_antib = 3;%antibodies Decay Time
miu_f=1/T_antib;
Recovery= 30;%organ Recovery Period 20 for organism is needed 30 days for recoverying
miu_m=1/Recovery; 
C_st=200;
F_st=rho*C_st/miu_f/2;
...
m(1)=0;
for i=2:n
    C(i)=C(i-1)+dt*(sin(m(i-1)*pi/2)*alpha*V(i-1)*(dt*i-tau)*F(i-1)-miu_c*(C(i-1)-C_st));
